    ...
    <div class="container-fluid d-flex">
        ...
        <div class="p-2" id="mynavbar">
            <ul class="navbar-nav me-auto">
                @if($race['distributedPoints'] == 1)
                    <li class="nav-item" style="margin-left: 25px;">
                        <a class="nav-link" href="{{ route('showScoresOfRace', $race['id']) }}">Classificació</a>
                    </li>
                @else
                    <li class="nav-item" style="margin-left: 25px;">
                        <a class="nav-link" href="{{ route('showPreInscription', $race['id']) }}">Inscripció</a>
                    </li>
                @endif
            </ul>
        </div>
...
<div id="demo" class="carousel slide" data-bs-ride="carousel" style="height: 600px;">
    <div class="carousel-inner">
        <div class="carousel-item active">
            <img src="../images/races_images/{{$race['promoteInfo']}}" alt="Los Angeles" class="d-block w-100" style="height: 500px !important;">
        </div>
    </div>
</div>
<div class="container mt-6">
    <h1 class="raceTitle">{{$race['name']}}</h1>
    <h2 style="padding-bottom: 10px; margin-left: 15px;">Serveis i inscripcions</h2>
        <div class="d-flex justify-content-between" style="margin-left: 45px;">
            <div class="d-flex flex-column">
                <p class="text">Descripció: {{$race['description']}} </p>
                <p class="text">Dia: {{$race['date']}}</p>
                <p class="text">Hora: {{$race['dateTime']}}</p>
            </div>
        </div>
    <h2 style="padding-bottom: 10px; margin-left: 15px;">Recorregut</h2>
        <div class="mapInfo" style="margin-left: 45px;">
            <iframe iframe src="{{$race['map']}}" width="680" height="480" allowfullscreen></iframe>
            <p class="text" style="margin-top: 16px">Distancia: {{$race['km']}} KM</p>
            <p class="text">Desnivell: {{$race['unveness']}}</p>
        </div>
</div>
