...
@section('content')
    ...
    <div class="container mt-6"
        style="display: flex; justify-content: center; align-items: center; flex-direction: column; width: 50%; padding: 20px">
        <h1 class="raceTitle">Formulari d'inscripció: {{ $race['name'] }}</h1>
        <form action="{{ route('inscriptionValidateFormOpen', $race['id']) }}" method="POST" style="width: 100%">
            @csrf
            ...
            <div class="form-group mb-4">
                <label for="insurer" class="col-sm-2 col-form-label">Assegurança</label>
                <select class="form-select" aria-label="Default select example" name="insurer">
                    <option selected>Escollir asegurança ...</option>
